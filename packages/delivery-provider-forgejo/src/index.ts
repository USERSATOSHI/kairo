...
export interface ForgejoPullRequestProviderOptions {
...
function parse(
  ...
  if (
    ...
    return err({
      ...
      code: 'forgejo_invalid_pull_request',
      message: 'Forgejo returned a malformed pull request',
    });
  }
  return ok({
    ...input,
...
export class ForgejoPullRequestProvider implements PullRequestProvider {
  readonly id = 'forgejo' as const;
  private readonly apiUrl: string;
  private readonly requestFetch: DeliveryFetch;

  constructor(private readonly options: ForgejoPullRequestProviderOptions) {
    this.apiUrl = `${options.instanceUrl.replace(/\/+$/, '')}/api/v1`;
  ...
  async find(
    ...
    const query = new URLSearchParams({ state: 'all', head: input.head, base: input.base });
    ...
    if (!Array.isArray(response.value)) {
      return err({
        kind: DeliveryErrorKind.InvalidResponse,
        code: 'forgejo_invalid_pull_request_list',
        message: 'Forgejo returned a malformed pull-request list',
      });
    }
  ...
  private async request(
    ...
    if (!response.ok) {
      return err({
        ...
        code: `forgejo_http_${response.status}`,
        message: `Forgejo pull-request request failed with status ${response.status}`,
      });
    }
    return fromAsync(
